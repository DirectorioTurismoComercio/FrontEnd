...
angular.module('map')
    .controller('MapController', function ($scope, $window, uiGmapGoogleMapApi, uiGmapIsReady, SearchForResultsFactory,
                                           ...
                                           $rootScope, $resource, RatingService, API_CONFIG) {
        ...
        function showSiteFromUrl() {
            ...
            if (siteId) {
                ...
                SiteDAO.get({siteId: siteId}, function (site) {
                    $scope.foundSites = [site];
                    centerMapOnSearchedTown();
                    if ($scope.foundSites != undefined) {
                        MapRouteService.setSiteMarker($scope.foundSites, $scope);
                    }
                }, function (error) {
                    messageService.showErrorMessage("SITE_NOT_FOUND");
                });
            }
        }
